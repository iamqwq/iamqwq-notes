...
如果在原有一颗二叉搜索树的情况下不断添加越来越大的数，就会出现二叉树的最坏构造情况，即变成线性表，这时时间复杂度会趋于 O(n)，效率会出现降低的情况。

为避免这种效率降低的情况，需要将二叉树的结构尽量保持平衡，避免出现 “ 一条线 ” 的结构；不难想到，我们需要在二叉树进行插入的时候针对左右子树进行平衡性的调整。

为了表示二叉树的平衡程度，首先需要引出平衡因子：

<img src="Tree%20-%20%E6%A0%91.assets/202302181549balance-factor.excalidraw.png" alt="202302181549blance-factor.excalidraw" style="zoom: 33%;" />

该图右侧的数字代表树的层次，节点中的数字代表数据，节点上方的数字代表平衡因子。平衡因子的计算方式是：计算左右子树的深度的绝对值，当左右子树均为空时平衡因子为 0。当平衡因子小于 2 时则说树是平衡的，若平衡因子大于或等于 2 时则说树是不平衡的。
