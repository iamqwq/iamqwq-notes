...
为解决这个问题，我们可以考虑一种新的，更加易于组织函数和字段、理清代码之间的关系的思想来作为编写代码的原则，这个原则在 Java 中就是面向对象。面向对象思想中有两个基本概念：对象和类。

对象不是一种计算机专有的名词，从更根本的角度来思考对象的意义更有助于理解面向对象的思想：哲学中的对象，又称客体，它是任何可以被思考或讨论的事物。对象具有一些**具体的**属性（例如一个红苹果是对象，具有颜色的值（红色）和市场价格的值（或许是四块一斤））。

将若干个具有相同特征的对象抽象成一种概念，也就是对它们定义为一 " 类 " 事物。

对象和类之间有着固定的关系，即 X 是 Y（例如水果是有定义的一类事物，而苹果作为具体的对象**是**一种水果）。也可以说，对象就是类的某个具体实例。以对象的方式来组织代码（变量和操作），这就是面向对象。面向对象思想让我们在代码中简洁的描述对象和对象的关系（因为对象是类的具体实例，因此描述对象和对象的关系前，我们就要设计类和类的关系）。

在计算机中更多的把下至硬件上至整个应用的任何事物看作对象，甚至把应用中完成某一职责的部分看作对象也是常见的。注意，对象在粒度上在不同的场景中有不同的大小，我们既可以把很宽泛（包含了若个事物）的事物看作一个对象，也可以把很细微（很难再拆分成出某个事物）的事物看作一个对象。
