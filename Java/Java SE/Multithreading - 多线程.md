...
在讲线程之前就不得不提到进程这个概念，进程又是相对于程序这个概念。程序是一组可执行的代码，是一个静态的概念；如果将程序运行，就是一个进程，是一个动态的概念。

在系统中存在很多的进程，他们是并行运行的。而线程可以理解为进程的拓展概念，一个进程可以存在多个线程，而不同线程又是互相并行的。

**如何并行？**现如今 CPU 大抵都为多核心，进程和线程都通过占用不同的核心，以达到每个核心同时各自处理不同的内容的效果。若没有那么多（或空闲的）核心可供使用，则将时间进行切片，每个时间切片的实际时间长度都极短，CPU 在每个不同时间切片都执行不同的计算工作。
...
6. 终止状态 （ TERMINATED 终止 ）
   ...
   当 run 方法中的代码执行完毕，则线程为该状态。

尽管网上和 《 Java 核心技术 》 中有各式的图解六种线程状态，但廖雪峰的网站中的示意图仍让我眼前一亮：
...
实际上，Java 规范没有将运行作为一种单独的状态。当调用 start 方法，线程就处于可运行状态，这意味着正在运行的线程也处于可运行状态。需要注意的是，
...
多个线程之间有时需要对同一块数据进行操作。如果线程间竞争资源时，如果对资源的访问顺序敏感，则存在竞态条件，导致竞态条件发生的代码区称为临界区。竞态条件会产生预期之外的结果，情况如下图所示：
